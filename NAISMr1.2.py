...
class NAISM1:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            c1 = tf.Variable(tf.truncated_normal(shape=[self.num_items, self.embedding_size], mean=0.0, stddev=0.01),
    ...
    def _attention_MLP(self, q_):  #b*n*e
        with tf.name_scope("attention_MLP"):
            ...
            b = tf.shape(q_)[0]
            n = tf.shape(q_)[1]
            # tile_size = tf.constant([b, 1, 1], tf.float32, [3]) #[b,1,1]
            ...
            A_softmax = tf.expand_dims( frac * self.exp_A_, 2) #b*n*1

            return tf.reduce_sum(A_softmax * q_, 1)
...
def training(model, dataset, batch_size, epochs, num_negatives):
    ...
    with tf.Session() as sess:
        ...
        while epoch_count < epochs:
            ...
            saver.save(sess, 'checkpoints/NAISM1', index)
            index += 1
...
def training_batch(t, epoch, index, model, sess, data):
    ...
    user_input, num_idx, item_input, labels, mask = data.batch_gen(index)
...
if __name__=='__main__':
    ...
    model = NAISM1(dataset.num_items, args.batch_size, args.lr, args.embed_size, args.alpha, regs)
    model.build_graph()
