...
def parse_args():
    parser = argparse.ArgumentParser(description="Run NAIS_concat.")
...
name='c1', dtype=tf.float32, trainable=trainable_flag)
            ...
            self.W = tf.Variable(tf.truncated_normal(shape=[2*self.embedding_size, self.embedding_size], mean=0.0, stddev=tf.sqrt(tf.div(2.0, 3*self.embedding_size))),
            ...
            self.h = tf.Variable(tf.truncated_normal(shape=[self.embedding_size, 1], mean=0.0, stddev=0.01),
...
def training(flag, model, dataset, batch_size, epochs, num_negatives):
    logging.info("begin training the NAIS_concat model...")
...
if __name__=='__main__':
    ...
    args = parse_args()
    regs = eval(args.regs)
    logging.basicConfig(filename="Log/NAIS_concat/log_lr%.4f_bs%d_%s_%.2f_pre%d" %(args.lr, args.batch_size, args.dataset, args.alpha, args.pretrain), level = logging.INFO)
    dataset = Dataset(args.path + args.dataset)
    model = NAIS_concat(dataset.num_items,args.dataset,args.pretrain, args.batch_size, args.lr, args.embed_size, args.alpha, regs)
    model.build_graph()
